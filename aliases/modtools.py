...
@register_alias("getlocation")
def getlocation(plugin, event, args):
    loc = event.getPlayer().getLocation()
    event.getPlayer().sendMessage('{x},{y},{z} {pitch},{yaw}'.
                                  format(x=loc.getBlockX()), y=loc.getBlockY(), z=loc.getBlockZ(),
